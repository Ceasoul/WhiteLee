# Contributing to ReTale

## The highest-value contribution: a new game adapter

1. Read `retale/adapters/base.py` — the contract is one method,
   `extract(source, protagonist_hint) -> ExtractionResult`.
2. Copy `retale/adapters/dota2_opendota.py` as a template.
3. Map your game's events onto the `EventKind` vocabulary in
   `retale/core/schema.py`. Resist adding new kinds — nuance belongs in
...
5. `pytest -q && ruff check retale/`, then open a PR.
...
`retale/narrative/`, something is wrong.
