# WhiteLee — 把你的游戏过程变成文学作品

**Turn your gameplay into literature.**

WhiteLee is a game-session-to-novel engine. Point it at a match you played —
...
它不是流水账日志：WhiteLee 会先重建整场比赛的**叙事弧**（开端、危机、高潮、
...
fiction: **causally coherent plots nobody pre-authored.** WhiteLee mines that
...
whitelee dota2 8123456789 --pov "YourNick" --style wuxia -o saga.md

# CS2: local demo file (pip install -e .[cs2])
whitelee cs2 mymatch.dem --pov YourName --style hardboiled

# Inspect the chapter plan without spending tokens
whitelee dota2 8123456789 --dry-run
```
...
```bash
whitelee dota2 8123456789 --style adventure --style-sample my_essays.txt
```
...
| env | value |
|---|---|
| `WHITELEE_PROVIDER` | `anthropic` (default) / `openai` / `openai_compatible` |
| `WHITELEE_MODEL` | override model name |
| `WHITELEE_BASE_URL` | for local models: Ollama, vLLM, DeepSeek endpoints |
...
writing one adapter** (`whitelee/adapters/base.py` is the whole contract).
...
`whitelee/adapters/base.py` (the contract is one method), copy
